...
### matrepair
Tool to compute a grammar compressed matrix multiplication friendly version of a matrix given in `.csv` format. Calls the tools `mat2vc` and `irepair0` (below). With option `-r`, shows a nice report detailing running times and compression ratio. 
