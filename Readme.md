...
### remm
Tool to compute a series of left and right matrix-vector multiplications. Takes as input a repair compressed matrix (files `.val`, `.vc.R`, `.vc.C`) of size *RxC*, a vector *x* of size *C* stored in a binary file (possibly generated by `makevec.py`) and an integer parameter *n* and computes *n* times the operation *y=Mx*, *z=y M^t*, *x = z/|z|*.
